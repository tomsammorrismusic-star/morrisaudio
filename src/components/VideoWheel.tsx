  ...
  return (
    <>
            ...
            {[...videoItems, ...videoItems].map((item, index) => (
              <div key={`${item.id}-${index}`} className="flex-shrink-0 w-64 overflow-visible">
                <button
                  ...
                  className="w-full h-56 rounded-3xl bg-gradient-to-br from-gray-300 to-gray-200 border border-orange-500 flex flex-col items-center justify-center gap-3 cursor-pointer hover:border-yellow-400/50 transition-colors shadow-xl group bubble-hover"
                >
                  <div className={`w-full h-full rounded-3xl bg-gradient-to-br ${CATEGORY_COLORS[item.category] ?? 'from-gray-300 to-gray-200'} border border-orange-500 flex flex-col items-center justify-center gap-3 cursor-pointer hover:border-yellow-400/50 transition-colors shadow-xl relative overflow-hidden`}>
      ...
      {selectedItem && (
        ...
        >
          <div
            className="relative bg-brand rounded-3xl border border-orange-500 shadow-2xl max-w-2xl w-full"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close button */}
            <button
              onClick={() => setSelectedItem(null)}
              className="absolute top-4 right-4 p-2 text-gray-800 hover:text-white bg-lime-700 rounded-2xl z-10 transition-colors"
            ...
            <div className="p-8 bg-brand">
              ...
              <div className="flex items-center justify-between gap-5">
                <button
                  ...
                  className="p-2 text-gray-800 hover:text-lime-700 transition-colors bubble-hover"
