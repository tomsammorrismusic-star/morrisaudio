  ...
  return (
    <>
      ...
      {selectedItem && (
        ...
        >
          <div
            className="relative bg-brand rounded-3xl border border-lime-600 shadow-2xl max-w-2xl w-full"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close button */}
            <button
              onClick={() => setSelectedItem(null)}
              className="absolute top-4 right-4 p-2 text-red-300 hover:text-yellow-100 bg-lime-700 rounded-2xl z-10 transition-colors"
            ...
            <div className={`w-full h-96 rounded-t-xl bg-gradient-to-br ${CATEGORY_COLORS[selectedItem.category] ?? 'from-gray-300 to-gray-200'} flex flex-col items-center justify-center gap-5 relative overflow-hidden`}>
              <div className="w-20 h-20 rounded-full bg-brand/20 flex items-center justify-center">
                <Play className="w-8 h-8 text-yellow-100 fill-white ml-1" />
              </div>
            </div>

            {/* Content section */}
            <div className="p-8 bg-brand">
              <h3 className="text-3xl font-semibold mb-2 text-yellow-100">{selectedItem.title}</h3>
              ...
              <div className="flex items-center justify-between gap-5">
                <button
                  ...
                  className="p-2 text-red-300 hover:text-lime-700 transition-colors bubble-hover"
