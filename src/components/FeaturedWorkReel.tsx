    ...
    >
      ...
      >
        ...
        {[...videos, ...videos].map((item, index) => (
          ...
          >
            <button
              className={`relative w-64 h-64 rounded-3xl bg-gradient-to-br ${CATEGORY_COLORS[item.category] ?? 'from-gray-300 to-gray-200'} border border-orange-600 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-red-500 transition-all duration-500 ease-out group bubble-hover hover:shadow-2xl hover:scale-110`}
            ...
            >
              <div className="w-12 h-12 rounded-full bg-brand/20 flex items-center justify-center group-hover:bg-brand/30 transition-all duration-300">
                <Play className="w-5 h-5 text-gray-900 fill-white ml-0.5" />
              </div>
              <div className="text-center px-3">
                <p className="text-gray-900 font-semibold text-xs leading-tight line-clamp-2">{item.title}</p>
              </div>
            </button>
          </div>
        ))}
