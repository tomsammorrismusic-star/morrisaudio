...
function Resume() {
  return (
    <div className="min-h-screen bg-brand text-yellow-100 p-8 lg:p-12">
      <div className="max-w-4xl mx-auto space-y-12">
        ...
        {/* Career Summary */}
        <Card className="bg-lime-700 border-lime-600 text-yellow-100">
          <CardHeader>
            <CardTitle className="text-2xl text-yellow-100">Career Summary</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col md:flex-row items-start gap-8">
              <p className="flex-1 leading-relaxed text-red-200">
        ...
        <section className="space-y-6">
          <h2 className="text-3xl font-semibold text-yellow-100">Work Experience</h2>
          <div className="space-y-6">
            {allJobs.map((job) => (
              <Card key={job.jobTitle} className="bg-lime-700 border-lime-600 text-yellow-100">
                <CardHeader>
                  <div className="flex justify-between items-start flex-wrap gap-3">
                    <div className="space-y-1">
                      <CardTitle className="text-xl text-yellow-100">{job.jobTitle}</CardTitle>
                      <p className="font-medium text-lime-700">
                        {job.company} · {job.location}
                      </p>
                    </div>
                    <Badge variant="secondary" className="text-sm bg-gray-200 text-red-200">
                      {job.startDate} – {job.endDate || 'Present'}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent>
                  <p className="mb-8 leading-relaxed text-red-200">{job.summary}</p>
                  <div className="flex flex-wrap gap-2">
                    {job.tags.map((tag) => (
                      <HoverCard key={tag}>
                        ...
                        <HoverCardContent className="w-64 bg-lime-700 border-lime-600 text-gray-800">
        ...
        {/* Education */}
        <section className="space-y-6">
          <h2 className="text-3xl font-semibold text-yellow-100">Education</h2>
          <div className="space-y-6">
            {allEducations.map((education) => (
              <Card key={education.school} className="bg-lime-700 border-lime-600 text-yellow-100">
                <CardHeader>
                  <CardTitle className="text-xl text-yellow-100">{education.school}</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="leading-relaxed text-red-200">{education.summary}</p>
                  {education.content && (
                    <div
                      className="mt-6 prose prose-sm max-w-none"
